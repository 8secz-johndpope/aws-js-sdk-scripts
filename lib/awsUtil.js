    ...
    return new Promise((resolve, reject) => {
        s3.listBuckets(function (err, data) {
            if (err) {
                ...
                let existingBuckets = Object.keys(Consts.STAGE_BUCKETS).map((env)=>Consts.STAGE_BUCKETS[env]);
                let targetBucket = data.Buckets.find((bucket) => existingBuckets.some((stageBucket) => bucket.Name.includes(stageBucket)));
